---
title: "EndPoints CTL08: Metabolism Gene Signatures"
...
params: 
    ...
    OutputFolder: '~/DataDir/bulkRNASeq/4.GeneSignatures/CTL08/Output/AfterSamplesSelection/MetabolismSig/'
    CpmFilt: 2
    SampleFilt: 4
...
```{r, collapse=TRUE}
GS <- read.table(file = '~/DataDir/bulkRNASeq/4.GeneSignatures/GeneSets/MetabolismGenes.txt', 
...
```{r , collapse=TRUE, fig.width=15, fig.height=12}
...
tidyHeatmap::save_pdf(p1, filename=paste0(OutputFolder, 'LogFpkm.pdf'), width=15, height=12)
```
...
```{r , collapse=TRUE, include=FALSE}
tidyHeatmap::save_pdf(p2, filename=paste0(OutputFolder, 'ScaledLogfpkm.pdf'), width=15, height=12)
```
...
```{r , collapse=TRUE, include=FALSE}
tidyHeatmap::save_pdf(p3, filename=paste0(OutputFolder, 'LogFC.pdf'), width=15, height=12)
```
...
Date <- date()
save.image(paste0(OutputFolder, '/', Dataset, 'SigMetab_Workspace.RData'))
```
