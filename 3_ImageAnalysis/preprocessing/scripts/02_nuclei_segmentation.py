...
for scene in img.scenes:
    ...
    for i in df_vertices['index'].unique():
        ...
        single_mask_tif = crop_image(mask, region[0].bbox, offset = 0)
        
        labels_tot, _ = model.predict_instances(normalize(single_organoid_tif), n_tiles = model._guess_n_tiles(single_organoid_tif))
        labels_tot = np.where(single_mask_tif, labels_tot, 0)
        
        tif.imwrite(f'{output_path}/mask_nuclei/{image_name}_{scene}_rep_{i}_organoid_mask.tif', labels_tot) 
