...
```{r, collapse=TRUE}
GS <- read.table(file = '~/DataDir/ExternalData/Receptors/EndocrineKeyGenes.txt', 
                         sep='\t', header=TRUE)
```
...
```{r logfpkm, collapse=TRUE, fig.width=5, fig.height=12}
...
```{r scaledlogfpkm, collapse=TRUE, fig.width=5, fig.height=12}
...
```{r scaledlogcpm, collapse=TRUE, fig.width=5, fig.height=12}
...
```{r logFC,    collapse=TRUE, fig.width=5, fig.height=12}
...
```{r MeanLogFpkm_lolli, collapse=TRUE, fig.height=12, fig.width=6}
ColVec <- c('DMSO' = '#4D4D4D', 'AH' = '#F8766D', 'AND'= '#fccb17', 'EST' = '#53B400', 'GC' = '#00C094', 'LX' = '#00B6EB', 'RA' = '#A58AFF', 'THY' = '#FB61D7')
#scales::hue_pal()(12)
...
CuratedSig$Lolli <- LolliPopCols(Data=CuratedSig$LogFpkmMean, PointSize=2.5,
                                                   ...
                                                   Shape=15, GeneSize=7, filterZero=FALSE) + theme(
...
```{r DeltaLogFpkmvsDay25_lolli, collapse=TRUE, fig.height=12, fig.width=6}
CuratedSig$LolliDelta <- LolliPopCols(CuratedSig$LogFpkmDelta, PointSize=3, SegSize=1.5,
                                            cols=ColVec, yLow=-3, yHigh=4,  CeilUp=4, CeilDown=-3, 
                                            unit='DeltaLogFpkm', Shape=18, GeneSize=7, filterZero=FALSE) +
                                                theme(
                                                      legend.text = element_text(size = 12),
                                                      legend.title = element_text(size = 14)
                                                    ) + labs(color = "Pathway")
...
```{r DeltaLogFpkmvsDay0_lolli, collapse=TRUE, fig.height=12, fig.width=6}
CuratedSig$LolliDeltaII <- LolliPopCols(CuratedSig$LogFpkmDeltaII, PointSize=3, SegSize=1.5,
...
Saving in Pdf
```{r, collapse=TRUE}
ggsave(plot=CuratedSig$Lolli, filename=paste0(OutputFolder, 'Cheroni_LogFpkm.pdf'), width=5, height=10)
ggsave(plot=CuratedSig$Lolli, filename=paste0(OutputFolder, 'Cheroni_LogFpkm.png'), width=5, height=10)

ggsave(plot=CuratedSig$LolliDelta, filename=paste0(OutputFolder, 'Cheroni_DeltaLogFpkm_vs25.pdf'), width=5, height=10)
ggsave(plot=CuratedSig$LolliDelta, filename=paste0(OutputFolder, 'Cheroni_DeltaLogFpkm_vs25.png'), width=5, height=10)
```
